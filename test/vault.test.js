...
describe("entry encryption", () => {
	...
	it("detects a flipped ciphertext byte via entry SHA3-256 (corruption, not keyed tampering)", () => {
		...
		assert.throws(() => decryptEntry(key, "CORRUPT_ME", flipped), (error) => {
	...
	it("rejects a ciphertext swapped into another entry's slot (reference is GCM AAD)", () => {
		...
		assert.throws(() => decryptEntry(key, "REF_B", a), (error) => {
...
describe("document serialization and startup verification", () => {
	...
	it("is stable regardless of map insertion order", () => {
		...
		const text1 = serializeDocument(new Map([["A", recordA], ["B", recordB]]));
		const text2 = serializeDocument(new Map([["B", recordB], ["A", recordA]]));
	...
	it("rejects structurally invalid documents", () => {
		...
		assert.throws(() => parseDocument(JSON.stringify({ version: 99, algorithm: VAULT_ALGORITHM, sha3: "", entries: {} })), { code: "VAULT_INVALID" });
		assert.throws(() => parseDocument(JSON.stringify({ version: VAULT_VERSION, algorithm: "rot13", sha3: "", entries: {} })), { code: "VAULT_INVALID" });
		assert.throws(() => parseDocument(JSON.stringify({ version: VAULT_VERSION, algorithm: VAULT_ALGORITHM, sha3: "", entries: { "9bad": {} } })), { code: "VAULT_INVALID" });
		// structurally valid entry, wrong fingerprint → corruption, not invalidity
		assert.throws(() => parseDocument(JSON.stringify({ version: VAULT_VERSION, algorithm: VAULT_ALGORITHM, sha3: "", entries: { GOOD: { data: "x", sha3: "y" } } })), { code: "VAULT_CORRUPTED" });
	});
...
describe("password-derived master keys", () => {
	...
	it("derives a key, round-trips the password key file, and never stores the password", async () => {
		...
		const derived = await deriveMasterKey(PASSWORD, salt);
	...
	it("different salts derive different keys from the same password", async () => {
		const a = await deriveMasterKey(PASSWORD, generatePasswordSalt());
		const b = await deriveMasterKey(PASSWORD, generatePasswordSalt());
	...
	it("rejects empty passwords, empty salts, and garbage key files", async () => {
		...
		await assert.rejects(deriveMasterKey(PASSWORD, Buffer.alloc(0)), { code: "MASTER_KEY_INVALID" });
		...
		await assert.rejects(parsePasswordKeyFile(JSON.stringify({ kdf: "pbkdf2" }), PASSWORD), { code: "MASTER_KEY_INVALID" });
	});
...
describe("single-file encrypted credential store", () => {
	...
	it("detectCredentialStore separates the two forms", async () => {
		const created = await encryptCredentialStore(new Map([["KEY", "value"]]), PASSWORD);
	...
	it("encrypts in place: ciphertext without plaintext, unlock round-trips", async () => {
		const plaintexts = new Map([["A_KEY", "sk-a"], ["B_KEY", "sk-b"]]);
		...
		const unlocked = await unlockEncryptedStore(created.text, PASSWORD);
	...
	it("rejects a wrong password with PASSWORD_WRONG before any entry is touched", async () => {
		const created = await encryptCredentialStore(new Map([["KEY", "value"]]), PASSWORD);
		await assert.rejects(unlockEncryptedStore(created.text, "wrong password"), { code: "PASSWORD_WRONG" });
		zeroizeBuffer(created.key);
	});
	...
	it("tampering a ciphertext entry is VAULT_CORRUPTED with the reference named", async () => {
		const created = await encryptCredentialStore(new Map([["CORRUPT_ME", "value"]]), PASSWORD);
		...
		forged.sha3 = createHash("sha3-256").update(JSON.stringify(Object.fromEntries([
			["format", forged.format], ["version", forged.version], ["algorithm", forged.algorithm],
			["kdf", forged.kdf], ["n", forged.n], ["r", forged.r], ["p", forged.p],
			["salt", forged.salt], ["verifier", forged.verifier], ["entries", forged.entries]
		])), "utf8").digest("hex");
		assert.throws(() => parseEncryptedStore(`${JSON.stringify(forged, null, 2)}\n`), (error) => {
			assert.equal(error.code, "VAULT_CORRUPTED");
			assert.match(error.message, /CORRUPT_ME/);
			return true;
		});
	...
	it("rejects empty passwords and structurally invalid stores", async () => {
		...
		assert.throws(() => parseEncryptedStore(JSON.stringify({ format: "dsh-encrypt-credentials" })), { code: "VAULT_INVALID" });
		assert.throws(() => parseEncryptedStore(JSON.stringify({ format: "dsh-encrypt-credentials", version: 99, algorithm: VAULT_ALGORITHM, kdf: "scrypt", n: 1, r: 1, p: 1, salt: "", verifier: {}, entries: {}, sha3: "" })), { code: "VAULT_INVALID" });
	});
});
