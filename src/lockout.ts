...
export const DEFAULT_LOCKOUT_BASE_MS = 30000
/** Default lockout window ceiling (15 min). */
export const DEFAULT_LOCKOUT_MAX_MS = 900000
...
export interface LockoutResult extends LockoutState {
  retryAfterMs: number
}
 ...
 */
export function lockoutRetryAfterMs(lockedUntil: number, now: number = Date.now()): number {
 ...
 */
export function isLockedOut(lockedUntil: number, now: number = Date.now()): boolean {
...
export function recordLockoutFailure(
  ...
  const maxAttempts =
    Number.isInteger(config?.maxAttempts) && config.maxAttempts > 0 ? config.maxAttempts : DEFAULT_MAX_ATTEMPTS
  const baseMs =
    Number.isFinite(config?.lockoutBaseMs) && config.lockoutBaseMs > 0 ? config.lockoutBaseMs : DEFAULT_LOCKOUT_BASE_MS
  const maxMs =
    Number.isFinite(config?.lockoutMaxMs) && config.lockoutMaxMs > 0 ? config.lockoutMaxMs : DEFAULT_LOCKOUT_MAX_MS
  ...
  return { failures, lockedUntil, retryAfterMs: Math.max(0, lockedUntil - now) }
}
