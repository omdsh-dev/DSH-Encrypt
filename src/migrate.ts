...
async function assertOwnerOnly(filename) {
  try {
    ...
    if (process.platform !== "win32" && (mode & GROUP_OTHER_BITS) !== 0) {
      throw new Error(`dsh-encrypt: ${filename} is readable beyond its owner (mode ${(mode & 0o777).toString(8)}); run "chmod 600 ${filename}" before migrating`);
    }
  } catch (error) {
 ...
 */
export async function migrateLegacySidecar(options = {}) {
  if (typeof options.path !== "string" || options.path.length === 0) throw new TypeError("dsh-encrypt: migration requires a legacy credentials path");
  const filename = resolve(options.path);
  const sidecar = resolve(options.encryptedPath ?? (
  ...
  return withFileLock(filename, async () => {
    ...
    if (detectCredentialStore(text) !== "encrypted") throw new Error(`dsh-encrypt: ${filename} is not a legacy encrypted store`);
