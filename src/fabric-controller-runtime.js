...
export function resolveSpec(config = {}) {
  ...
  const encryptedFilename = resolve(
...
async function assertOwnerOnly(filename) {
  try {
    ...
    if (process.platform !== 'win32' && (mode & GROUP_OTHER_BITS) !== 0) {
      throw new Error(
        `dsh-encrypt: ${filename} is readable beyond its owner (mode ${(mode & 0o777).toString(8)}); run "chmod 600 ${filename}" before starting again`,
      )
    }
  } catch (error) {
...
export class EncryptController {
    ...
    if (sidecar !== void 0) {
      if (primary !== void 0 && !isEncryptedMarker(primary)) {
        ...
        if (plain.size > 0) {
          throw new VaultError(
            'VAULT_INVALID',
            `${this.spec.filename} contains plaintext while the encrypted sidecar is active; restore the encrypted marker before starting`,
          )
        }
      ...
      if (primary !== void 0 && isEncryptedMarker(primary)) {
        throw new VaultError(
          'VAULT_INVALID',
          `the encrypted marker exists but ${this.spec.encryptedFilename} is missing`,
        )
      }
      if (this.stateEncrypted === true) {
        throw new VaultError(
          'VAULT_MIGRATION_REQUIRED',
          `the encrypted state for ${this.spec.filename} has no sidecar; migrate the legacy vault before starting`,
        )
      }
      this.format = 'plain'
      this.unlocked = true
  ...
  afterResolve(ref, result) {
    ...
    if (result?.source === 'file') {
      ...
      throw new VaultError(
        'VAULT_INVALID',
        `the official credentials file ${this.spec.filename} is not the encrypted marker`,
      )
    }
    ...
    if (!this.unlocked || this.key === void 0) {
      throw new VaultError(
        'VAULT_LOCKED',
        `the encrypted credential store is locked; unlock it in Settings → 加密安全 (or export ${this.spec.passwordEnv})`,
      )
    }
  ...
  afterDescribe(ref, result) {
    ...
    if (result !== void 0 && !(result.configured === false && result.source === void 0 && this.entries.has(ref)))
      return result
  ...
  async setPassword(digest) {
    ...
    return this.enqueue(async () => {
      ...
      return withFileLock(this.spec.filename, async () => {
        ...
        await writeFileAtomic(this.spec.filename, encryptedMarker(this.spec.encryptedFilename), {
          mode: 0o600,
          dirMode: 0o700,
        })
        this.format = 'encrypted'
  ...
  async changePassword(oldDigest, digest) {
    if (!isDigest(oldDigest) || !isDigest(digest)) {
      throw new VaultError(
        'PASSWORD_INVALID',
        'the current and new password digests must be 64 lowercase hex characters (SHA3-256 of the password)',
      )
    }
    this.assertUnlocked('change the password')
    return this.enqueue(async () =>
      withFileLock(this.spec.encryptedFilename, async () => {
  ...
  async reconcileFromDisk() {
    ...
    if (this.format !== 'encrypted') {
      ...
      if (marker !== void 0 && !isEncryptedMarker(marker)) {
        ...
        if (plain.size > 0) {
          ...
          throw new VaultError(
            'VAULT_INVALID',
            `${this.spec.filename} contains plaintext while the encrypted sidecar is active`,
          )
        }
        await this.writeMarker()
    ...
    if (sidecar === void 0) {
      ...
      throw new VaultError(
        'VAULT_CORRUPTED',
        `the encrypted sidecar ${this.spec.encryptedFilename} disappeared; keeping the last trusted snapshot`,
      )
    }
    if (marker === void 0 || !isEncryptedMarker(marker)) {
      this.plaintextForbidden = true
  ...
  async setRememberDays(days) {
    if (!Number.isInteger(days) || days < -1 || days > 30)
      throw new VaultError('PASSWORD_INVALID', 'remembered-login days must be an integer from -1 through 30')
  ...
  async loadStateFile() {
    ...
    if (doc === null || typeof doc !== 'object' || Array.isArray(doc)) {
      throw new VaultError(
        'STATE_INVALID',
        `the dsh-encrypt state file ${this.spec.stateFile} must contain a JSON object`,
      )
    }
    if (
      doc.rememberDays !== void 0 &&
      (!Number.isInteger(doc.rememberDays) || doc.rememberDays < -1 || doc.rememberDays > 30)
    ) {
      throw new VaultError(
        'STATE_INVALID',
        `the dsh-encrypt state file ${this.spec.stateFile} has an invalid rememberDays value`,
      )
    }
    if (doc.encrypted !== void 0 && doc.encrypted !== true) {
      throw new VaultError(
        'STATE_INVALID',
        `the dsh-encrypt state file ${this.spec.stateFile} has an invalid encrypted value`,
      )
    }
    if (doc.unlockFailures !== void 0 && (!Number.isInteger(doc.unlockFailures) || doc.unlockFailures < 0)) {
      throw new VaultError(
        'STATE_INVALID',
        `the dsh-encrypt state file ${this.spec.stateFile} has an invalid unlockFailures value`,
      )
    }
    if (doc.unlockLockedUntil !== void 0 && (!Number.isFinite(doc.unlockLockedUntil) || doc.unlockLockedUntil < 0)) {
      throw new VaultError(
        'STATE_INVALID',
        `the dsh-encrypt state file ${this.spec.stateFile} has an invalid unlockLockedUntil value`,
      )
    }
  ...
  async persistState() {
    try {
      ...
      const doc = {}
      if (Number.isInteger(this.stateRememberDays) && this.stateRememberDays >= -1 && this.stateRememberDays <= 30)
        doc.rememberDays = this.stateRememberDays
  ...
  withUnlockedBuffer(ref, fn) {
    return (async () => {
      ...
      const buffer =
        this.format === 'encrypted' && this.key !== void 0
