...
import { patchStubs } from '../lib/stent-handlers.js'

const root = resolve(fileURLToPath(new URL('..', import.meta.url)))
const patchFile = resolve(root, 'cordis.patch.yml')
const document = parse(await readFile(patchFile, 'utf8'))
const row = document.flatMap(operation => operation?.insert ?? []).find(entry => entry?.id === 'dsh-encrypt-stent')
if (row === void 0) throw new Error('dsh-encrypt: cordis.patch.yml is missing dsh-encrypt-stent')
if (row.disabled !== true) throw new Error('dsh-encrypt: dsh-encrypt-stent must remain disabled outside stent-dsh')
...
if (JSON.stringify(actual) !== JSON.stringify(expected)) {
  throw new Error('dsh-encrypt: cordis.patch.yml Stent descriptors drift from lib/stent-handlers.js')
}
