window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *
		 * The remembered-login ticket also travels as an explicit
		 * `x-dsh-encrypt-remember` header (localStorage-backed), so the
		 * auto-unlock does not depend on browser cookie storage quirks; the
		 * HttpOnly cookie remains the fallback path.
		 */
		...
		function pingStatus() {
			void apiPostRaw("/api/credentials.status").then((body) => {
				if (body?.ok === true && body.value?.ticketRejected) storeTicket(null);
			}).catch(() => {});
		}
		...
		function EncryptionSection(_props) {
			...
			const refresh = React.useCallback(async () => {
				try {
					...
					if (!body.ok) throw new Error(body.message || body.code);
					if (body.value?.ticketRejected) storeTicket(null);
