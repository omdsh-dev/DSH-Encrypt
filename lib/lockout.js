 ...
 */
export function lockoutRetryAfterMs(lockedUntil, now = Date.now()) {
 ...
 */
export function isLockedOut(lockedUntil, now = Date.now()) {
 ...
 */
export function recordLockoutFailure(state, config, now = Date.now()) {
	const maxAttempts = Number.isInteger(config?.maxAttempts) && config.maxAttempts > 0 ? config.maxAttempts : DEFAULT_MAX_ATTEMPTS;
 ...
 */
export function formatLockoutMessage(retryAfterMs) {
