 ...
 *   are further pinned to a loopback connection (Host AND socket).
...
export function redactingHttpHandler(handler, guard) {
	return async (req, res) => {
		...
		res.end = (chunk, encoding, callback) => {
			...
			const enc = typeof encoding === "string" ? encoding : void 0;
			if (chunk !== void 0 && chunk !== null) {
				// Push FIRST, then flush: flushing before the push would swallow
				// a short one-shot body (res.end(chunk)) — the pushed chunk would
				// sit in the held tail with nothing left to emit it.
				const pieces = stream.push(chunk);
...
/** Largest accepted request body in bytes (digests are 64 hex chars; 64 KiB is generous). */
const MAX_BODY_BYTES = 64 * 1024;

/** Wrap one password operation into a JSON HTTP handler with the write fence. */
function jsonHandler(operation, logger) {
	return async (req, res) => {
		...
		for await (const chunk of req) {
			...
			if (bytes > MAX_BODY_BYTES) return write(413, { ok: false, code: "payload-too-large", message: "request body exceeds 64 KiB" });
			raw += chunk;
		}
		let payload = {};
		try {
		...
		try {
			...
			// VaultError messages are safe by contract; anything else may carry
			// filesystem paths or internal detail, so it is logged server-side
...
export function apply(ctx, config) {
					...
					if (ticket !== void 0) {
						try {
							...
							if (error?.code === "REMEMBER_EXPIRED" || error?.code === "REMEMBER_INVALID") {
								ctx.logger.warn("dsh-encrypt: remembered-login ticket rejected (%s); clearing the cookie", error.code);
								cookies.clear();
								ticketRejected = true;
	...
	for (const { path, operation } of routes) {
		ctx.effect(() => ctx.webServer.register({ kind: "exact", path, handler: jsonHandler(operation, ctx.logger) }), `dsh-encrypt: ${path}`);
	}
}
