...
import { VaultError, REMEMBER_DAY_MS, SCRYPT_N, SCRYPT_P, SCRYPT_R, createRememberBlock, decryptEntry, detectCredentialStore, encryptCredentialStore, encryptEntry, isDigest, parseEncryptedStore, recoverKeyFromRemember, rememberActive, serializeEncryptedStore, sha3_256Hex, unlockEncryptedStore, zeroizeBuffer } from "./vault.js";
...
var EncryptedCredentialProvider = class extends CredentialProvider {
	...
	async loadInitial() {
		...
		if (detectCredentialStore(text) === "encrypted") {
			...
			if (ambient !== void 0 && ambient.value.length > 0) {
				await this.applyUnlock(await unlockEncryptedStore(text, sha3_256Hex(ambient.value)));
			}
			return;
		}
		this.format = "plain";
	...
	/** The WebUI-facing snapshot: form, lock state, and remembered-login state. */
	status() {
	...
	async unlock(digest) {
		...
		this.applyUnlock(await unlockEncryptedStore(this.text, digest));
		return {
			unlocked: true
		};
	}
	 ...
	 * entry is re-encrypted under the new derived key, and the old
	 * remembered-login block dies with the old key.
	 * @param digest - lowercase hex SHA3-256 digest of the new password.
	 */
	async changePassword(digest) {
		if (!isDigest(digest)) throw new VaultError("PASSWORD_INVALID", "the password digest must be 64 lowercase hex characters (SHA3-256 of the password)");
		...
		return this.enqueue(async () => {
			...
			await withFileLock(this.spec.filename, async () => {
				...
				if (this.format === "encrypted") {
					...
					const nextText = serializeEncryptedStore(next, this.key, this.salt, this.params, this.remember);
	...
	async loadStateFile() {
		...
		if (typeof doc === "object" && doc !== null && !Array.isArray(doc) && Number.isInteger(doc.rememberDays) && doc.rememberDays >= -1 && doc.rememberDays <= 30) {
			this.stateRememberDays = doc.rememberDays;
		} else {
			this.ctx.logger.warn("dsh-encrypt: ignoring invalid rememberDays in %s", this.spec.stateFile);
		}
	}
	...
	async setRememberDays(days) {
		...
		return this.enqueue(async () => {
			...
			await writeFileAtomic(this.spec.stateFile, `${JSON.stringify({ rememberDays: days }, null, 2)}\n`, { mode: 384, dirMode: 448 });
			this.stateRememberDays = days;
			if (this.format !== "encrypted") return null;
			await withFileLock(this.spec.filename, async () => {
				await this.reconcileFromDisk();
				if (this.remember !== void 0) {
					const text = serializeEncryptedStore(this.entries, this.key, this.salt, this.params);
	...
	async persistRemember(days) {
		...
		await withFileLock(this.spec.filename, async () => {
			...
			const text = serializeEncryptedStore(this.entries, this.key, this.salt, this.params, created.block);
			await writeFileAtomic(this.spec.filename, text, { mode: 384, dirMode: 448 });
			this.text = text;
