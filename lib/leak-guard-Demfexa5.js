//#region src/security/redaction/literal-matcher.ts
/**
	...
	find(text) {
		...
		while (start < text.length) {
			...
			while (cursor < text.length) {
				...
				node = next;
				cursor += 1;
				if (node.value !== void 0) longest = {
					start,
					end: cursor,
					value: node.value
				};
			}
			if (longest === void 0) start += 1;
			else {
...
const REDACTION_MARKER = "[REDACTED:dsh-encrypt]";
/**
...
var LeakGuard = class {
	...
	*/
	constructor(options = {}) {
		...
		this.marker = options.marker ?? "[REDACTED:dsh-encrypt]";
	}
	...
	*/
	scan(text) {
		if (text.length === 0 || !this.enabled || this.#values.size === 0) return {
			text,
			matched: 0,
			refs: []
		};
		...
		for (const match of matches) {
			...
			if (entry !== void 0) {
				for (const ref of entry.refs) if (!refs.includes(ref)) refs.push(ref);
			}
		}
...
var RedactionStream = class {
	...
	*/
	push(chunk) {
		...
		const matches = matcher.find(combined);
	...
	*/
	flush() {
		...
		const tail = this.#tail;
		this.#tail = "";
		const matches = matcher.find(tail);
		let output = tail;
		let shift = 0;
		for (const match of matches) {
...
*/
function buildServerFrame(opcode, payload) {
	const body = typeof payload === "string" ? Buffer.from(payload, "utf8") : Buffer.from(payload);
	let header;
	if (body.length < 126) header = Buffer.from([128 | opcode & 15, body.length]);
	...
	*/
	push(chunk) {
		...
		for (;;) {
			...
			if (frame === null) break;
			if (frame.passthrough) throw new Error("dsh-encrypt: unsupported or invalid server WebSocket frame");
			const { fin, opcode, headerLen, payloadLen, raw, rest } = frame;
			if (((raw[0] ?? 0) & 112) !== 0) throw new Error("dsh-encrypt: WebSocket extension frames are not accepted by the redaction layer");
			const payload = raw.subarray(headerLen, headerLen + payloadLen);
			this.#buffer = rest;
			if (opcode === 1) {
				if (this.#textFragments !== null || this.#binaryFragment) throw new Error("dsh-encrypt: invalid nested WebSocket data message");
				if (!fin) {
			...
			if (opcode === 0 && this.#textFragments !== null) {
				this.#textFragmentBytes += payload.length;
				if (this.#textFragmentBytes > 8388608) throw new Error("dsh-encrypt: fragmented WebSocket text message exceeds 8 MiB");
...
//#endregion
export { buildServerFrame as a, WsFrameFilter as i, REDACTION_MARKER as n, readServerFrame as o, RedactionStream as r, LeakGuard as t };

//# sourceMappingURL=leak-guard-Demfexa5.js.map