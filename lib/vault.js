 ...
 *
 * The key file may hold either a legacy raw 32-byte key or the digest
 * format: scrypt parameters + a random salt + an AEAD verifier. The user's
 * password itself never crosses the wire and is never stored — the WebUI
 * derives its SHA3-256 digest, and `deriveMasterKey()` stretches that
 * 64-hex digest into the key on every start; `parsePasswordKeyFile()` proves
 * the digest by decrypting the verifier record (`PASSWORD_WRONG` on
 * mismatch). Forgetting the password means wiping the vault; nothing else
...
/** Document schema version. v2 stretches the password's SHA3-256 digest. */
...
/** KDF input marker: scrypt stretches the lowercase hex SHA3-256 digest. */
 ...
 * password with scrypt. Only the digest (64 lowercase hex characters) is
 * accepted: the browser derives it from the password, so the raw password
 * never reaches this process. The digest itself is never stored or logged;
 * only the salt and an AEAD verifier are persisted.
 ...
 */
export async function deriveMasterKey(digest, salt, params = { n: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P }) {
...
const STORE_FIELDS = ["format", "version", "algorithm", "kdf", "kdfInput", "n", "r", "p", "salt", "verifier", "remember", "entries"];
 ...
 * @returns {string} the document text.
 */
export function serializeEncryptedStore(records, key, salt, params = { n: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P }, remember) {
	if (!Buffer.isBuffer(key) || key.length !== MASTER_KEY_BYTES) throw new VaultError("MASTER_KEY_INVALID", "a master key must be a 32-byte buffer");
	if (!Buffer.isBuffer(salt) || salt.length === 0) throw new VaultError("MASTER_KEY_INVALID", "a password salt must be a non-empty buffer");
...
export function parseEncryptedStore(text) {
	...
	if (doc.version !== VAULT_VERSION) throw new VaultError("VAULT_INVALID", `unsupported store version ${JSON.stringify(doc.version)} (expected ${VAULT_VERSION}); a store encrypted by dsh-encrypt ≤ 0.1.0-rc.6 must be unlocked and returned to plaintext with that version first`);
 ...
 * @param {string} text - the raw document text.
 * @param {string} digest - the lowercase hex SHA3-256 digest of the password.
 * @returns {Promise<{ key: Buffer, entries: Map<string, EntryRecord>, salt: Buffer, params: Object, remember: Object|undefined }>}
 */
export async function unlockEncryptedStore(text, digest) {
	const parsed = parseEncryptedStore(text);
	const key = await deriveMasterKey(digest, parsed.salt, parsed.params);
	try {
	...
	return { key, entries: parsed.entries, salt: parsed.salt, params: parsed.params, remember: parsed.remember };
}
 ...
 */
export async function encryptCredentialStore(plaintexts, digest) {
	if (!isDigest(digest)) throw new VaultError("PASSWORD_INVALID", "the password digest must be 64 lowercase hex characters (SHA3-256 of the password)");
	const salt = generatePasswordSalt();
	const key = await deriveMasterKey(digest, salt);
