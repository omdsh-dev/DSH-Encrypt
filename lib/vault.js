 ...
 *
 * The key file may hold either a legacy raw 32-byte key or the password
 * format: scrypt parameters + a random salt + an AEAD verifier. The user's
 * password is never stored — `deriveMasterKey()` stretches it into the key
 * on every start, and `parsePasswordKeyFile()` proves the password by
 * decrypting the verifier record (`PASSWORD_WRONG` on mismatch). Forgetting
 * the password means wiping the vault with `tools/reset.mjs`; nothing else
 * can recover it, by design.
 *
...
/** scrypt cost parameters for password-derived master keys (OWASP-aligned). */
export const SCRYPT_N = 131072;
export const SCRYPT_R = 8;
export const SCRYPT_P = 1;
/** scrypt memory ceiling in bytes; 128 * N * r needs 128 MiB plus overhead. */
...
/**
 * Derive a 32-byte master key from a user-chosen password with scrypt. The
 * password itself is never stored or logged; only the salt and an AEAD
 * verifier are persisted, so the derived key exists in memory alone.
 * @param {string} password - the non-empty password.
 ...
 */
export async function deriveMasterKey(password, salt, params = { n: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P }) {
	if (typeof password !== "string" || password.length === 0) throw new VaultError("PASSWORD_INVALID", "a password must be a non-empty string");
	...
	return scryptAsync(password, salt, MASTER_KEY_BYTES, { N: params.n, r: params.r, p: params.p, maxmem: SCRYPT_MAXMEM });
}

/**
 * Serialize a password-protected key file: scrypt parameters, the salt, and
 * an AEAD verifier the password is checked against. Contains neither the
 * password nor the derived key.
 * @param {Buffer} key - the derived 32-byte key.
 * @param {Buffer} salt - the salt the key was derived from.
 * @param {{ n: number, r: number, p: number }} params - the cost parameters used.
 * @returns {string} the key-file text.
 */
export function createPasswordKeyFile(key, salt, params = { n: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P }) {
	if (!Buffer.isBuffer(key) || key.length !== MASTER_KEY_BYTES) throw new VaultError("MASTER_KEY_INVALID", "a master key must be a 32-byte buffer");
	const doc = {
		version: 1,
		kdf: "scrypt",
...
export function isPasswordKeyFile(text) {
	try {
		...
		return isRecord(doc) && doc.kdf === "scrypt";
	} catch {
		return false;
	}
}
...
export async function parsePasswordKeyFile(text, password) {
	...
	if (!isRecord(doc) || doc.kdf !== "scrypt" || typeof doc.salt !== "string") throw new VaultError("MASTER_KEY_INVALID", "unsupported password key-file format");
...
/**
 * Decrypt one record into its plaintext. Entry integrity is verified first
 * (SHA3-256), then GCM authentication. The returned plaintext is a transient
 * value: callers must use it within the operation and drop the reference.
 * @param {Buffer} key - the 32-byte master key.
...
//     "version": 1, "algorithm": "aes-256-gcm+sha3-256",
//     "kdf": "scrypt", "n": ..., "r": ..., "p": ...,
//     "salt": "<base64url>",
//     "verifier": { "data": ..., "sha3": ... },   ← AEAD password check
...
const STORE_FIELDS = ["format", "version", "algorithm", "kdf", "n", "r", "p", "salt", "verifier", "entries"];
 ...
 * a password can be checked without touching any credential.
 * @param {Map<string, EntryRecord>} records - ciphertext records keyed by reference.
 * @param {Buffer} key - the derived 32-byte key.
 * @param {Buffer} salt - the persisted scrypt salt.
 * @param {{ n: number, r: number, p: number }} params - the cost parameters used.
 * @returns {string} the document text.
 */
export function serializeEncryptedStore(records, key, salt, params = { n: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P }) {
	if (!Buffer.isBuffer(key) || key.length !== MASTER_KEY_BYTES) throw new VaultError("MASTER_KEY_INVALID", "a master key must be a 32-byte buffer");
	if (!Buffer.isBuffer(salt) || salt.length === 0) throw new VaultError("MASTER_KEY_INVALID", "a password salt must be a non-empty buffer");
	const doc = {
 ...
 * integrity check, distinct from unlocking.
 * @param {string} text - the raw document text.
 * @returns {{ salt: Buffer, params: { n: number, r: number, p: number }, verifier: EntryRecord, entries: Map<string, EntryRecord>, documentSha3: string }}
 */
export function parseEncryptedStore(text) {
	...
	if (doc.version !== VAULT_VERSION) throw new VaultError("VAULT_INVALID", `unsupported store version ${JSON.stringify(doc.version)} (expected ${VAULT_VERSION})`);
	...
	if (doc.kdf !== "scrypt") throw new VaultError("VAULT_INVALID", `unsupported store kdf ${JSON.stringify(doc.kdf)}`);
...
/**
 * Unlock an encrypted credential store with the user's password: derive the
 * key, check it against the AEAD verifier (`PASSWORD_WRONG` on mismatch), and
 * hand back the key plus the ciphertext entries. Entries are decrypted per
 * operation by the caller — plaintext never sits in the unlocked snapshot.
 * @param {string} text - the raw document text.
 * @param {string} password - the password to check.
 * @returns {Promise<{ key: Buffer, entries: Map<string, EntryRecord> }>}
 */
export async function unlockEncryptedStore(text, password) {
	const parsed = parseEncryptedStore(text);
	const key = await deriveMasterKey(password, parsed.salt, parsed.params);
	try {
		decryptEntry(key, KEY_FILE_VERIFIER_REF, parsed.verifier);
	} catch (error) {
		zeroizeBuffer(key);
		if (error instanceof VaultError && (error.code === "VAULT_KEY_MISMATCH" || error.code === "VAULT_CORRUPTED")) throw new VaultError("PASSWORD_WRONG", "the password does not match this credential store");
		throw error;
	}
	return { key, entries: parsed.entries, salt: parsed.salt, params: parsed.params };
}

/**
 * Encrypt a full plaintext credential map under a brand-new password: draws
 * the salt, derives the key, encrypts every entry, and serializes the
 ...
 * @param {string} password - the new non-empty password.
 * @returns {Promise<{ text: string, key: Buffer }>} the document text and the derived key (zeroize when done).
 */
export async function encryptCredentialStore(plaintexts, password) {
	if (typeof password !== "string" || password.length === 0) throw new VaultError("PASSWORD_INVALID", "a password must be a non-empty string");
	...
	return { text: serializeEncryptedStore(records, key, salt), key, entries: records, salt };
}
