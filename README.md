...
- **免密登录滑块**：设置「多少天免密」（0 = 每次都输密码 / 1–30 天 / 永远，刻度仅保留头尾），解锁成功后签发 256 位票据，**默认仅经 HttpOnly Cookie 下发**（响应体不回传、浏览器无 JS 可读副本）；`rememberChannel: "header"` 可显式改回响应体回传 + localStorage + `x-dsh-encrypt-remember` 请求头通道（XSS 可读票据，仅用于 Cookie 存取异常的 WebView）；**打开 WebUI 即自动尝试免密解锁**（本机专属；页面加载、标签页重新可见或重新聚焦时都会自动重试，服务重启后切回页面即可，无需整页刷新），磁盘只存被 AEAD 包裹的密钥副本；到期或票据不匹配即失效
...
滑块即改即存（松开即提交到 `/api/credentials.config`，只接受本机 Host；刻度只显示「每次」与「永远」两头）。解锁成功后按当前滑块时长签发免密票据（默认仅 HttpOnly Cookie）；改动滑块会作废旧票据，若当前已解锁则立即按新时长重新签发。**打开 WebUI 时自动尝试免密解锁**（本机专属；标签页重新可见/聚焦也会重试，服务重启后切回页面即可，无需整页刷新），免密期间无需输密码；到期后回到密码解锁。**非本机访问始终需要输入密码，且不显示改密入口**（后端同样强制）。
