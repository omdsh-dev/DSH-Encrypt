...
- **SHA3-256 双重完整性**:条目级指纹 + 覆盖文档头部的文档级指纹,损坏文件启动即被拒绝(绝不当作“空库”)
...
- **发行代码完整性自校验**:打包时生成 `lib/integrity-manifest.json`(全部发行文件 SHA3-256),启动逐文件校验、不一致即拒绝加载(fail-closed);哈希对行尾/BOM 归一化,git 克隆与 tarball 安装校验一致
- **免密登录滑块**:0 = 每次输密码 / 1–30 天 / 永远;解锁成功后签发 256 位票据,**默认仅 HttpOnly Cookie**(响应体不回传);页面加载、**标签页重新可见/聚焦**时自动重试解锁,服务重启后切回页面即可
- **仅本机免密与改密**:「本机」= Host 回环 **且** socket 回环双重判定(LAN 客户端伪造回环 Host 会被 socket 校验拦截)
...
```sh
npm pack                       # prepack 自动重建 lib/integrity-manifest.json
...
`@node-rs/argon2` 为带预编译二进制的原生依赖,极少平台组合缺失时 `npm rebuild @node-rs/argon2` 即可。
...
- **忘记密码**:不可恢复(设计如此);唯一恢复手段是删除 `.credentials.yaml` 后重新配置
## 凭证解析顺序
...
```json
{
  ...
  "remember": { "salt": "<base64url>", "issuedAt": 1755000000000, "days": 7,
...
- `remember` 块仅在签发过免密票据时存在:`cipher` 是票据密钥 AEAD 包裹的主密钥;票据本身只在浏览器 Cookie(HttpOnly),永不落盘;`days: -1` 表示永远
- 文档级指纹覆盖 `sha3` 以外全部字段;条目级指纹覆盖各自密文 blob;成本参数有上限(防恶意构造耗尽内存/CPU)
- 密码与票据从不落盘

## HTTP 路由(web 行)

仅 `POST application/json`(与官方 /api 相同的跨站写护栏),全部路由先过 **Host 头信任围栏**(防 DNS rebinding):Host 必须解析为回环主机名或命中 `trustedHosts`,否则 **403 `FORBIDDEN_HOST`**;改密/设密/免密设置进一步**钉死本机**(Host 回环 **且** socket 回环,不给 `trustedHosts` 例外)。请求体上限 64 KiB(超限 413)。响应 `{ ok: true, value }` 或 `{ ok: false, code, message }`,错误消息不含密码或任何密钥材料:

| 路径 | 请求体 | 作用 |
| :-- | :-- | :-- |
| `/api/credentials.status` | `{}` | 状态快照;本机且携带免密票据(Cookie;header 通道下也接受 `x-dsh-encrypt-remember`)时先尝试票据解锁 |
| `/api/credentials.unlock` | `{ digest }` | 密码摘要解锁;本机成功后再按滑块签发免密票据(HttpOnly Cookie;header 通道才随响应体回传票据);失败过多返回 429 |
...
| 字段 | 类型 | 默认值 | 说明 |
| :-- | :-- | :-- | :-- |
| `trustedHosts` | string[] | `[]` | 除回环外放行的权威来源(host 或 host:port),与 client-connection 的 `trustedHosts` 一致 |
...
- 永远密文:设密后无法回退明文
- POSIX 上凭证文件必须 owner-only(0600),否则拒绝启动;Windows 由创建 API 与 OS ACL 表达
- 防爆破:失败计数持久化 + 指数退避 + 429/`Retry-After`
- 防注入回显(Leak Guard):解析过的凭证在 HTTP 响应与 WS 文本帧离开主机前被替换为 `[REDACTED:dsh-encrypt]`(流式脱敏,绝不先漏出前缀)
- 防 DNS rebinding(Host 围栏)与防 LAN 伪造回环(Host+socket 双重判定)
- 改密需证明当前口令(`oldDigest`),且与解锁共用锁定计数(锁定窗口期间改密同样 429 拒绝)
- 防篡改/防逆向侧:发行代码 SHA3-256 完整性自校验,任何字节不一致即拒绝加载
...
- 摘要即口令等价物:捕获一次即可解锁直到改密;明文 HTTP 传输,回环绑定仅暴露给本机进程,绑定 LAN 请前置 TLS 反向代理(本插件不自带 TLS)
...
| `INTEGRITY_FAILED` | 发行代码完整性校验失败(拒绝加载;合法重建后 `npm run integrity`) |
...
| 插件 | `D:\Developments\DSH\DSH-Encrypt`(源码 + 自持依赖树 + 自己的 package-lock.json,junction 挂进 profile) | 与 dsh 完全无关,独立开发/发布 |
...
```sh
npm install        # 自包含依赖树;接缝包精确钉版,独立包范围版
npm test           # node --test:91 个单元测试(vault / web / trust / provider / lockout / leak-guard / compat;pretest 自动重建完整性清单)
npm run integrity  # 手工重建 lib/integrity-manifest.json(修改任何 lib/ 文件后必须执行)
npm pack           # prepack 自动重建清单后打包
```

安全测试覆盖:解锁锁定期望(阈值/指数退避/上限/不复位)、Host 围栏与回环双重判定(DNS rebinding 403 + LAN 伪造回环 Host 被 LOCAL_ONLY)、票据通道(cookie/header)、请求体上限(413)与内部错误脱敏(通用 500)、输出脱敏接线(注册前后均被包裹、明文不外泄、dispose 还原)、泄露检测(正则转义/最长优先/流式分块边界)、WS 帧过滤(文本替换+长度重编码/拆分帧/二进制控制帧直通)、完整性清单(篡改/缺失 fail-closed)、兼容护栏(同线警告/跨线 UNSUPPORTED_DSH/无运行时跳过)。

额外验证:

```sh
node test/client-smoke.mjs                                            # 浏览器包 ModuleLoader/SSR 冒烟
node test/e2e-webui.mjs --base http://localhost:3199 --home <dsh-home> --phase 1   # 两阶段真实重启 e2e
